...
module.exports.getUniversities = async (req, res) => {
	try {
		...
		res.send({ status: httpStatus.OK, universities: universities, draw: draw, recordsTotal: total, recordsFiltered: stotal })
	} catch (error) {
		console.log(error)
		responseManagement.sendResponse(res, httpStatus.INTERNAL_SERVER_ERROR, global.internal_server_error);
	}
...
module.exports.editUniversity = async (req, res) => {
	try {
		let university = await University.findOne({ name: req.query._id });
		if (university) {
			responseManagement.sendResponse(res, httpStatus.BAD_REQUEST, global.university_already_exist);
		} else {
			console.log(university);
